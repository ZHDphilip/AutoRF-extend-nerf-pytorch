...
def create_AutoRF(args):
    """Instantiate AutoRF's model.
    """
    embed_fn, input_ch = get_embedder(args.multires, args.i_embed)
    ...
    output_ch = 5 if args.N_importance > 0 else 4
    skips = [4]
    model = AutoRF(pos_dim=input_ch, view_dir_dim=input_ch_views)
    grad_vars = list(model.parameters())
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        print(i, time.time() - t)
        t = time.time()
        rgb, disp, acc, _ = render(H[i], W[i], K[i], chunk=chunk, c2w=c2w[:3,:4], **render_kwargs)
...
def train():
    ...
    # Create nerf model
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer = create_AutoRF(args)
    global_step = start
